...
Game::Game()
{
    ...
    nextBlockArea.w = BLOCK_WIDTH * 4;
    nextBlockArea.h = BLOCK_WIDTH * 4;
    nextBlockArea.x = 480;
    nextBlockArea.y = 300;
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym)
                {
                    ...
                    case SDLK_ESCAPE:
                        ...
                        break;
                    case SDLK_z:
                        //store current tet (or swap, if one is already stored)
                        break;
                    default:
                        break;
                }
...
void Game::show_next_block()
{
    SDL_FillRect(screen, &nextBlockArea, SDL_MapRGB(screen->format, 50, 50, 50));
    ...
    switch (nextTet)
    {
        ...
        case O:
            blocks = OBlocks.at(0);
            colour = SDL_MapRGB(screen->format, 255, 0, 255);
            break;
        case T:
            blocks = TBlocks.at(0);
        ...
        case I:
            blocks = IBlocks.at(0);
            colour = SDL_MapRGB(screen->format, 160, 32, 240);
            break;
    }
    ...
    for (it=blocks.begin(); it<blocks.end(); it++)
    {
        ...
        blockFill.x = nextBlockArea.x + BLOCK_WIDTH * 2 + (*it).x * BLOCK_WIDTH;
        blockFill.y = nextBlockArea.y + BLOCK_WIDTH * 2 + (*it).y * BLOCK_WIDTH;
        if (nextTet == I)
            blockFill.y -= BLOCK_WIDTH;
        else if (nextTet == T || nextTet == J)
            blockFill.x -= BLOCK_WIDTH;
        blockFill.w = BLOCK_WIDTH - 2;
        blockFill.h = BLOCK_WIDTH - 2;

        SDL_FillRect(screen, &blockFill, colour);
    }
}
...
void Game::add_new_score(std::string playerName)
{
    //add new score to savefile, if score is high enough.
    std::list<std::string> names;
    std::list<int> scores;
    std::list<std::string>::iterator itstr;
    std::list<int>::iterator itint;

    load_high_scores(&names, &scores);
    ...
    {
        //if necessary, add in new score, and push others down (+ remove last entry)
        itint = scores.begin();
        itstr = names.begin();

        while (itint != scores.end())
        {
            if (currentScore > *itint)
            {
                scores.insert(itint, currentScore);
                names.insert(itstr, playerName);
                break;
            }
            itint++;
            itstr++;
